...
void SceneContext::DisplayWindowMessage()
{
    ...
    // glPushMatrix();
    // glLoadIdentity();
    // gluOrthof(0, mWindowWidth, 0, mWindowHeight, 1, 1000);
    // glMatrixMode(GL_MODELVIEW);
    // glPushMatrix();
    // glLoadIdentity();
    gl4duBindMatrix("projectionMatrix");
    gl4duLoadIdentityf();
    gl4duOrthof(0, mWindowWidth, 0, mWindowHeight, 1, 1000);
...
void SceneContext::DisplayGrid(const FbxAMatrix & pTransform)
{
    ...
    for (i = -hw; i <= hw; i+=bigstep)
    {
        ...
        // glRotatef(-90,1,0,0);
        gl4duPushMatrix();
        gl4duTranslatef(i+xoffset,0,zoffset);
        gl4duRotatef(-90,1,0,0);
        gl4duSendMatrices();
        mDrawText->Display(scoord.Buffer());
        // glPopMatrix();
        ...
        // glRotatef(-90,1,0,0);
        gl4duPushMatrix();
        gl4duTranslatef(xoffset,0,i+zoffset);
        gl4duRotatef(-90,1,0,0);
        gl4duSendMatrices();
        mDrawText->Display(scoord.Buffer());
        // glPopMatrix();
